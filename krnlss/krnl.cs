	...
	public class krnl : Form
	{
			...
			if (Directory.GetFiles("bin/tabs").Length != 0)
			{
				for (int j = 0; j < Directory.GetFiles("bin/tabs").Length / 2; j++)
				{
					...
					try
					{
						...
						((Control)(object)customTabControl1.GetWorkingTextEditor()).Text = streamReader2.ReadToEnd();
						streamReader2.Close();
					}
					catch
					{
			...
			try
			{
				...
				for (int j = 0; j < customTabControl1.TabPages.Count; j++)
				{
					...
					if (tabPage.Controls.Count > 0)
					{
						...
						File.WriteAllText($"bin/tabs/{j}_source.lua", ((Control)(object)val).Text.ToString());
					}
				}
			}
			catch
		...
		private void openIntoToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if ((dynamic)openFileDialog.ShowDialog() == DialogResult.OK)
			{
				...
				Control control = contextTab.Controls[0];
				Scintilla val2 = (Scintilla)(object)((control is Scintilla) ? control : null);
				((Control)(object)val2).Text = val;
		...
		private void bunifuFlatButton1_Click(object sender, EventArgs e)
		{
			try
			{
				Pipe(((Control)(object)customTabControl1.GetWorkingTextEditor()).Text);
			}
			catch (Exception ex)
			{
		...
		{
			((Control)(object)customTabControl1.GetWorkingTextEditor()).Text = "";
		}
		...
		private void bunifuFlatButton4_Click(object sender, EventArgs e)
		{
			...
			ScriptLoading();
			customTabControl1.OpenSaveDialog(customTabControl1.SelectedTab, ((Control)(object)customTabControl1.GetWorkingTextEditor()).Text);
		}
		...
		private void loadIntoEditorToolStripMenuItem_Click(object sender, EventArgs e)
		{
			try
			{
				dynamic fullPath = ScriptView.SelectedNode.FullPath;
				dynamic val = File.ReadAllText(Settings.Default.ScriptPath + "//" + fullPath);
				((Control)(object)customTabControl1.GetWorkingTextEditor()).Text = val;
			}
			catch (Exception)
			{
			}
		...
		private void krnl_FormClosing(object sender, FormClosingEventArgs e)
		{
			...
			try
			{
				...
				for (int j = 0; j < customTabControl1.TabPages.Count; j++)
				{
					...
					{
						Control control = tabPage.Controls[0];
						val = (Scintilla)(object)((control is Scintilla) ? control : null);
						File.WriteAllText($"bin/tabs/{j}_name.txt", tabPage.Text.ToString());
						File.WriteAllText($"bin/tabs/{j}_source.lua", ((Control)(object)val).Text.ToString());
					}
				}
